...
scp -o StrictHostKeyChecking=no -i ~/.ssh/$public_key helper_mysql.sh $username@$server_ip:/home/$username
ssh -o StrictHostKeyChecking=no -i ~/.ssh/$public_key $username@$server_ip "sudo ./helper_mysql.sh"
